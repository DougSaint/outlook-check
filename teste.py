...
K = '\033[1;35;40m'
V = '\033[1;36;40m'
turkuaz = '\033[1;36;40m'  # Turkuaz renk tanımı
reset = '\033[0m'  # Renk resetleme

######L7N#####
nnn = random.choice([R, X, F, B, K, V])
good_hot, bad_hot, good_ig, bad_ig, check, mj, ids = 0, 0, 0, 0, 0, 0, []
tok = input('• {}BOT TOKENİNİ GİR{} ♪ {} : {}'.format(B, C, V, K))
print("\r")
# Removido a solicitação de ID
os.system('clear')
...
def hunting(email):	
    ...
    'ig_sig_key_version': '4',
		...
		try:
			...
			rest = False
		try:
			info=requests.get('x'+email).json()
		except :
			info = None			
		try:
...
Número de posts: {post}
Redefinição de senha: {rest}
------------------------------------------
""")
		print(f"Resultados salvos em 'resultados.txt' para o usuário {email}")
	except :
		hunting(email)
